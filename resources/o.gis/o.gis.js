	...
	{						
		...
		// Get the geometry of the feature
		  ...
		  var paddedExtent = ol.extent.buffer(featureExtent, padding[0]);
		    ...
		    popup.show(geometry.getInteriorPoint().getCoordinates(), e.search);
						...
						if (isURL(value)) {
							...
							attributes[key].format = function (val, feature) {
							  ...
							  if (/\.(gif|jpg|jpeg|tif|tiff|png|avif|webp|svg)$/i.test(val)) {
								...
								const html = `
								<div class='popupimage' style="position: relative">
								<img id="${imgId}" src="images/${filename}">
								<button class="fa fa-expand" onclick="document.getElementById('${imgId}').requestFullscreen()"></button>
								</div>
								`;
								return html;
...
// Aggiorna il buffer dei layer sulla base dello zoom
	...
	// Quando ho finito di caricare i layers lancio il calcolo dell'extent
	document.addEventListener('DOMContentLoaded', () => {
		if (window.layersLoadedFlag) {
			setLayersExtent();
		} else {
			document.addEventListener('layersLoaded', setLayersExtent);
		}
		// Ad ogni moveend aggiorno l'extent
		...
		if (permalink.hasUrlParam('find')) {
			...
			if (selectedLayer) {
				...
				if (featureFound) {	
					...
					// Attendo che search.on sia pronto, quindi attendo che sia emesso searchReady
